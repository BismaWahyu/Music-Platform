...
export function MiniPlayer() {
  ...
  const seek = (e: ReactMouseEvent<HTMLDivElement>) => {
    ...
    setProgress(Math.max(0, Math.min(duration, ((e.clientX - r.left) / r.width) * duration)))
  }
      ...
      <div style={{ width: '380px', borderRadius: '26px', background: 'rgba(20,22,28,0.5)', backdropFilter: 'blur(50px) saturate(190%)', WebkitBackdropFilter: 'blur(50px) saturate(190%)', border: '1px solid rgba(255,255,255,0.12)', boxShadow: '0 40px 100px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.06)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '22px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div style={{ width: '100%', marginTop: '18px' }}>
            <div onClick={seek} style={{ position: 'relative', height: '5px', borderRadius: '3px', background: 'rgba(255,255,255,0.12)', cursor: 'pointer' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '26px', marginTop: '14px' }}>
            <Hover onClick={prev} style={{ cursor: 'pointer', color: '#c8cace' }} hover={{ color: '#fff' }}><PrevTrack size={20} /></Hover>
            <Hover onClick={togglePlay} style={{ width: '48px', height: '48px', borderRadius: '50%', background: '#f4f5f6', color: '#0b0c0e', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'transform 0.15s' }} hover={{ transform: 'scale(1.05)' }}>
              {isPlaying ? <PauseGlyph size={16} /> : <PlayGlyph size={16} />}
            </Hover>
            <Hover onClick={next} style={{ cursor: 'pointer', color: '#c8cace' }} hover={{ color: '#fff' }}><NextTrack size={20} /></Hover>
          </div>
        </div>
      </div>
    </div>
